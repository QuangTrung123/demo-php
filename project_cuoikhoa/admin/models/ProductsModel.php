<?php 
	trait ProductsModel{
		...
		public function modelUpdate($id){
			...
			$conn = Connection::getInstance();
			//thuc hien truy van
			$query = $conn->prepare("update products set name=:_name, description=:_description,content=:_content,hot=:_hot,category_id=:_category_id,price=:_price,discount=:_discount,amount =:_amount + amount where id=:_id");
			//thuc thi truy van. Neu khong co tham so o cau truy van thi ghi array rong
			$query->execute(array(":_id"=>$id,":_name"=>$name,":_description"=>$description,":_content"=>$content,":_hot"=>$hot,":_category_id"=>$category_id,":_price"=>$price,":_discount"=>$discount, ":_amount"=>$amount));
			//-------
			//neu user upload anh
		...
		public function modelRepair($id){
			//lay bien ket noi
			$conn = Connection::getInstance();
			//thuc hien truy van
			$query = $conn->prepare("select * from products where id=:_id");
		...
		public function modelCreate(){
			...
			$conn = Connection::getInstance();
			//thuc hien truy van
			$query = $conn->prepare("insert into products set name=:_name, description=:_description,content=:_content,hot=:_hot,photo=:_photo,category_id=:_category_id,price=:_price,discount=:_discount,amount=:_amount");
			//thuc thi truy van. Neu khong co tham so o cau truy van thi ghi array rong
			$query->execute(array(":_photo"=>$photo,":_name"=>$name,":_description"=>$description,":_content"=>$content,":_hot"=>$hot,":_category_id"=>$category_id,":_price"=>$price,":_discount"=>$discount,":_amount"=>$amount));
			//-------			
		}
