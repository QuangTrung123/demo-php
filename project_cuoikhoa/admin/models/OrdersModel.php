<?php 
	trait OrdersModel{
		...
		public function modelRead($recordPerPage){
			...
			$orderBy = "orders order by id,status desc ";
			switch($order){
				case "delivery":
				$orderBy = "where status = 1 order by id desc ";
				break;
				case "notdelivery":
				$orderBy = "where status = 0 order by id desc ";
				break;
				case "all":
				$orderBy = "orders order by id,status desc ";
				break;
			...
			$query = $conn->query("select * from orders $orderBy limit $from, $recordPerPage");
